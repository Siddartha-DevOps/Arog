...
const HEALTH_SCORE_CONFIG = {
  green: { label: "Healthy", bg: "bg-[#4CAF50]" },
  yellow: { label: "Moderate", bg: "bg-[#FFC107]" },
  red: { label: "Caution", bg: "bg-[#F44336]" },
};
...
export default function GlobalMealLogModal({ open, onClose, date, user }: Props) {
  ...
  const handleLog = async () => {
    ...
    setSaving(true);
    try {
      await logMeal({ date, mealType, description: description.trim(), ...result });
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-border">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground">for today</span>
              </div>
              <button
                onClick={handleClose}
                ...
                {result && (
                  ...
                  >
                    <div className="flex items-center justify-between p-4 rounded-xl border border-border bg-muted/40">
                      ...
                      <div className={`px-3 py-1 rounded-full text-white text-sm font-semibold ${score?.bg}`}>
                        {score?.label}
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {[
                        { label: "Protein", val: result.protein, color: "#2196F3" },
                        { label: "Carbs", val: result.carbs, color: "#FF9800" },
                        { label: "Fat", val: result.fat, color: "#4CAF50" },
                        { label: "Fiber", val: result.fiber, color: "#9C27B0" },
                      ].map((m) => (
                    ...
                    <div className="flex gap-2">
                      <Button variant="secondary" className="flex-1 cursor-pointer" onClick={() => setResult(null)}>
                        Re-analyze
                      </Button>
                      <Button
